...
const VOLATILITY_WINDOW = 12;   // was 10
const VOLATILITY_MIN    = 0.25; // was 0.20 (normalized % across window)
const ENTROPY_MAX       = 0.955;


const TRADE_COOLDOWN_MS = 1200;  // was 900
...
function dynamicVolatilityMin() {
  ...
  if (lastFast != null && lastSlow != null) {
    ...
    if (slope > 0.0005) req *= 0.90; // ~10% more permissive
  }
  return req;
}
...
function analyzeSignal() {
  ...
  // Volatility gate with detailed status
  if (!isMarketVolatile()) {
    setStatus(`Blocked: Low volatility (acc=${acc.toFixed(4)} < req=${reqVol.toFixed(4)})`, "#f59e0b");

    // Optional probe trade when bias is strong and entropy is acceptable
    const strongBias = Math.max(oddRatio, evenRatio) >= 75;
    const entropyOk = ent <= 0.85;
    if (strongBias && entropyOk) {
