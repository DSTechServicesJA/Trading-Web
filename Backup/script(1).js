...
// For normalized (percent) volatility; tune as needed.
// Example: 0.002 ≈ 0.2% cumulative across the window
const VOLATILITY_MIN = 0.20; 
const ENTROPY_MAX = 0.955;
let tradeMarkers = [];

const TRADE_COOLDOWN_MS = 900;
...
function dynamicVolatilityMin() {
  ...
  // If entropy is low (more bias), we can accept slightly lower vol
  const ent = digitEntropy();
  if (ent < 0.80) req *= 0.85; // ~15% more permissive

  // If EMAs are diverging (trend starting), be permissive
  ...
  if (lastFast != null && lastSlow != null) {
    ...
    if (slope > 0.0005) req *= 0.9; // ~10% more permissive
  }
  return req;
}
...
if (!isMarketVolatile()) {
  // compute current acc for display
  let acc = 0;
  ...
  const req = dynamicVolatilityMin ? dynamicVolatilityMin() : VOLATILITY_MIN;
  setStatus(`Blocked: Low volatility (acc=${acc.toFixed(4)} < req=${req.toFixed(4)})`, "#f59e0b");
  return false;
}
...
const strongBias = Math.max(oddRatio, evenRatio) >= 75;
const entropyOk = entropy <= 0.85;

// If bias is strong and entropy acceptable, permit a single probe trade
if (!isMarketVolatile() && strongBias && entropyOk) {
  setStatus("Probe: Strong bias despite low vol", "#22c55e");
  // Choose side based on stronger bias
  currentSide = oddRatio >= evenRatio ? CONTRACT_ODD : CONTRACT_EVEN;
  return true;
}
...
logoutBtn?.addEventListener("click", () => {
  ...
  // Try to forget subscriptions (best-effort)
  try {
    if (ws && ws.readyState === WebSocket.OPEN) {
      ws.send(JSON.stringify({ forget_all: ['ticks', 'balance', 'proposal', 'proposal_open_contract'] }));
    }
  } catch (e) {
