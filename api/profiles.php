...
try {
    ...
    error_log('profiles.php auth DB error: ' . $e->getMessage());
...
if ($method === 'GET') {
    try {
        ...
        $assigned->execute([$callerId, $callerId]);
...
if ($method === 'POST') {
    ...
    try {
        if ($profileId > 0) {
            ...
            $check->execute([$profileId, $callerId]);
            if (!$check->fetch()) {
            ...
            $existing->execute([$callerId, $name]);
            $row = $existing->fetch();
            ...
            if ($row) {
                ...
                $ins->execute([$name, $settingsJson, $callerId]);
...
if ($method === 'DELETE') {
    ...
    try {
        ...
        $del->execute([$profileId, $callerId]);
        if ($del->rowCount() === 0) {
            jsonResponse(['error' => 'Profile not found or not owned by you'], 404);
        }
